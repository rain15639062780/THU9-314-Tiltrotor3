	...
	if (count > 0) {
		for (uint16_t i = 0;i < count;i++) {
			if ((uint8_t)buf[i] == 0x55 && (uint8_t)buf[i+1] == 0xAA) {
				...
				sever_rxbuf.timer = (buf[i+4]) | (buf[i+5] << 8);
				sever_rxbuf.moto_num = (buf[i+6] | 0x03);
...
int sever_update_txbuf(void)
{
	...
	sever_txbuf.head1 = sever_txdata.head1;
	sever_txbuf.head2 = sever_txdata.head2;
	sever_txbuf.pos_l = sever_txdata.pos & 0xff;
	sever_txbuf.pos_h = (sever_txdata.pos  & 0xff00) >> 8;
	sever_txbuf.timer_l = sever_txdata.timer & 0xff;
	sever_txbuf.timer_h = (sever_txdata.timer & 0xff00) >> 8;
	sever_txbuf.num = sever_txdata.moto_num;
	sever_txbuf.checksum = sever_txdata.checksum;
...
int pi_uart_thread_main(int argc, char *argv[])
{
	...
	while(!thread_should_exit){
		...
		//	orb_publish(ORB_ID(sever_nozzle), sever_nozzle_rx_pub, &sever_rxdata);
			
			/* publish sever_nozzle_rxdata from uart*/
			
			if (sever_nozzle_rx_pub != NULL) {
			orb_publish(ORB_ID(sever_nozzle_rx), sever_nozzle_rx_pub, &sever_rxdata);
			
			} else {
			sever_nozzle_rx_pub = orb_advertise(ORB_ID(sever_nozzle_rx), &sever_rxdata);
			}
		...
		write(uart_fd,&sever_txbuf,sizeof(sever_txbuf));
